...
pub fn get_package_info(name: &str) -> Option<Package> {
    let result = run_command("paru", &["-Qi", name], false);
    if !result.success {
        return None;
    }
...
pub fn get_package_info_raw_available(name: &str) -> Option<String> {
    let result = run_command("paru", &["-Si", name], false);
    if result.success {
        Some(result.output)
    } else {
        None
...
pub fn search_packages(query: &str, max_results: usize) -> Vec<Package> {
    let result = run_command("paru", &["-Ss", query], false);
    if !result.success {
        return Vec::new();
    }
...
pub fn get_upgradable_packages() -> Vec<Package> {
    let result = run_command("paru", &["-Qu"], false);
    if !result.success {
        return Vec::new();
    }
...
pub fn install_package(name: &str) -> ExecutionResult {
    run_command("paru", &["-S", "--noconfirm", name], false)
}

pub fn remove_package(name: &str, recursive: bool) -> ExecutionResult {
    if recursive {
        run_command("paru", &["-Rsc", "--noconfirm", name], false)
    } else {
        run_command("paru", &["-R", "--noconfirm", name], false)
    }
}
...
pub fn clean_cache() -> ExecutionResult {
    run_command("paru", &["-Sc", "--noconfirm"], false)
}
